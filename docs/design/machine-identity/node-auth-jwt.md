...
must add validated, atomic credential-set promotion with rollback before planned
node CA rollover relies on this path. Until then, recovery from that failure
requires re-enrollment or another out-of-band restoration of a matching pair.
