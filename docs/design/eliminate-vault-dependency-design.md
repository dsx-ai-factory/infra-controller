...
| :---: | :---: | :--- | :--- |
| 1.0 | 2026-09-23 | Bill Minckler | High-level design for the Vault-free NICo runtime across credential storage, API authentication, and remaining key and certificate services |
...
- Replacing every API client's mTLS authentication with JWT. Phase 2 moves only
  Scout and DPU-agent machine authentication to bearer JWTs. The admin CLI has
  no JWT path and always authenticates with mTLS using a client certificate
  issued by an operator-managed non-Vault CA. Transport TLS and other
  certificate consumers also remain.
- Removing Scout and DPU-agent machine mTLS support. A JWT-capable deployment
  may disable that machine-client mTLS path, but it remains supported and is
  separate from the admin CLI's required mTLS path.
...
| UFM TLS certificates issued through `CreateCredential` with `CredentialType::Ufm` | Vault PKI through `CertificateProvider` | The same CSR-only cert-manager provider keeps the generated key out of Kubernetes and uses the existing issuing CA, ECDSA P-256 profile, existing SANs and usages, and 720-hour maximum lifetime; `CreateCredential` retains the restricted pod-local files used for manual installation on UFM and removes staging material after verified handoff and rollback | 3 |
| Service transport TLS for `nico-api`, `bmc-proxy`, `dhcp`, `dns`, `dsx-exchange-consumer`, `flow`, `hardware-health`, `machine-a-tron`, the optional `nico-site-health-probe`, `pxe`, and `ssh-console-rs`, plus the reference-installed site-agent and RMS | cert-manager `ClusterIssuer` `vault-nico-issuer`, signed by Vault PKI | Existing cert-manager `Certificate` resources, Secrets, certificate/key mounts, and `issuerRef` values remain in place; validation roots move from leaf-Secret `ca.crt` values to the versioned trust bundle, including the health probe's Core API CA input, and the existing issuer identity is converted in place to direct signing with the same CA for normal renewal | 3 |
...
| Optional `nico-api` NVSwitch mTLS certificates: NICo client and NMX-C/NVUE server | `nvSwitchTls.issuerRef` defaults to `vault-nico-issuer` | Existing cert-manager resources, Secrets, certificate/key mounts, and `issuerRef` values remain in place; NICo and switch validation roots use the versioned trust bundle before the existing issuer identity is converted in place to direct signing with the same CA for normal renewal | 3 |
...
3. cert-manager is the internal certificate-issuance control plane and uses the
   existing internal issuing CA without Vault. It has two issuance paths:

   - Service-transport, optional DPU OTLP, and optional NVSwitch certificates
     remain declarative cert-manager `Certificate` resources with their existing
     Secret and mount contracts.
   - Dynamic node and UFM certificates use a Rust provider. The provider creates
     each private key only in `nico-api` process memory and submits a CSR-only
     cert-manager `CertificateRequest`; the private key never enters a Kubernetes
     resource. A node certificate remains matched to its JWT-signing key and
     chains to the existing API trust anchor.
...
The [PostgreSQL secret-storage design](postgres-secrets/README.md) is
authoritative for the credential journal, envelope encryption, Vault import,
KEK routing, re-wrap, migration, and rollback mechanics. The
[Secrets Storage](../configuration/secrets-storage.md) and
[Credential Sources](../configuration/credential-sources.md) pages define the
operator-facing persistent and local-source contracts.

At the end of Phase 1, every `nico-api` credential consumer uses the
credential-provider abstraction. Credentials written through the API or
generated by NICo use envelope-encrypted PostgreSQL storage. Credentials
explicitly owned by local configuration use the documented environment, file,
or mounted-Secret sources. A provider that is not configured is not
constructed, so a Vault-free configuration does not create a Vault client.
Remaining credential consumers outside `nico-api` migrate in Phase 3.
...
- Before non-Vault renewal or CA rollover is enabled, the node credential
  ...
  staged generation and selects the last validated pair. A shared reader resolves
  the current generation once, opens every file from that immutable generation,
  validates the pair, and retries the entire read if promotion races it;
  `NodeJwtMinter` and `ForgeClientConfig` do not open independently resolved
  certificate and key paths. The JWT minter's key/certificate mismatch check
  remains defense in depth, not recovery from a crash or concurrent promotion.
...
DPU signing key into the BF3/BF4 fTPM. It is not required for the Vault-free
runtime. Certificate provisioning and lifecycle for the non-exportable key
remain unresolved and are discussed in [Section 7](#7-open-questions).
...
The CLI does not require JWT support. The existing
...
machine-certificate API response. For UFM, the current `CreateCredential` contract stages
...
Before the non-Vault provider or issuer cutover is enabled, the reference
...
the policies cover both the existing `vault-nico-issuer` compatibility identity
and the already-direct `site-issuer` so current renewal and rollback remain
available. Because disabling the built-in approver is cluster-wide, the policy
inventory also covers independent REST and
...
the provider and issuer cutover. The policies restrict requests to the intended
issuer, namespaces, identities, SAN patterns, usages, and lifetimes supported by the
...
existing cert-manager `Certificate` resources, Kubernetes Secrets, and exact
`issuerRef` values. The `vault-nico-issuer` name, kind, and group remain as a
compatibility identity while its backend changes in place from Vault PKI to the
CA issuer backed by `site-root`; their identities, SANs, usages, lifetimes, and
certificate/key mounts do not change. The optional DPU OTLP gateway certificate
already uses the direct `site-issuer` and remains in place. Its gateway-side DPU client validation and
...
certificate and signing key are available to the direct signing backend and
match the deployed chain. The REST `nico-rest-ca-issuer` path remains independent,
as described in [Section 2.1](#21-vault-touchpoints).
...
compatibility issuer is converted to direct signing. The reference chart does
not enable cert-manager Secret owner references. Preflight verifies that the
`site-root` Secret is not
...
reference are verified does setup create and restore-test a protected backup of
the CA certificate and private key, then remove the bootstrap `Certificate` and
issuer. The backup is encrypted in transit and at rest with a key held outside
the backup, and access is limited and audited as CA-signing access. It is retained
for the signer's full active lifetime and deleted only after that signer is
retired, all of its leaves have moved, and the rollback and recovery windows have
closed. The restore test proves the recovered key matches the CA certificate
without publishing a new signer. A fresh site creates its root without that owner
reference and applies the same backup and detachment procedure. Root renewal or
rekey is never an automatic cert-manager action.
...
Secrets, leaf certificates, and private keys. The implementation first stops
the current Helm hook from deleting and recreating `vault-nico-issuer`, then
updates that same `ClusterIssuer` object in place: its name, kind, group, and UID
remain stable while `spec.vault` is replaced in one API update by
`spec.ca.secretName: site-root`. No `Certificate` spec or issuer annotation in a
certificate Secret changes. Before the production update, an equivalent canary
issuer is switched from Vault to the same CA while its `Certificate` reference
is held constant; the cutover proceeds only if no new `CertificateRequest` is
created, the canary Secret's certificate and key digests and UID remain
unchanged, and the `Certificate` revision does not advance. A temporary
admission guard then rejects new requests owned by the
production `Certificate` resources. With the Vault issuer still active, the
migration lets every existing owned request finish, then requires every affected
`Certificate` to be ready and not issuing, with no pending request or staged next
private-key Secret. A request that cannot reach that state aborts the cutover,
leaves the Vault issuer unchanged, and removes and verifies removal of the
temporary guard so ordinary Vault-backed issuance can recover. Only then is the
production issuer updated. The guard is removed after that issuer is ready and
the same canary invariants hold. An attempted request or replaced
issuer object restores the Vault spec while Vault remains available; the guard
keeps the existing certificate Secrets unchanged until recovery completes.
...
the cutover, the migration validates that the candidate CA backend produces the
same CA chain and that the CA remains valid for the cutover and rollback window
plus the longest configured child-certificate lifetime. It stops rather than
...
site-agent, `nico-site-health-probe`, and other in-cluster API clients; both sides
of the optional DPU OTLP mTLS
...
installer and generation-aware reader are deployed on Scout and DPU-agent paths.
Each node promotes and verifies the new pair before its prior generation becomes
...
Loss or corruption of the `site-root` signing key first attempts the tested
protected restore while issuance is disabled. If no valid backup remains, the
public root stays in the trust bundle only for its still-valid leaves, and the
operator performs an emergency rollover to a new CA: distribute old-plus-new
trust, issue and verify replacement leaves through the new signer, then remove
the old root after no valid leaf depends on it. The system never generates a new
key under the old CA identity or treats loss as proof of compromise; suspected
disclosure follows the stricter compromise procedure above.
...
5. **Introduce the cert-manager-backed certificate provider.** Retain the
   ...
   certificate and signing key to be available to the direct signing backend,
   to match the active chain, and to have no bootstrap-Certificate owner reference that
   ...
   the compatibility issuer and already-direct `site-issuer` plus independent
   REST, DPF, and other supported cert-manager consumers, and install key-profile
   validation for every request path. Disable built-in automatic approval, then
   prove supported requests are approved and out-of-profile requests are denied.
   Keep the non-Vault provider and issuer cutover disabled until these checks
   pass. Create and restore-test the protected CA certificate and key backup,
   then detach the validated root Secret from automatic Certificate
   reconciliation and create its separate trust-bundle source. Route every
   internal validation-CA mount and config from
   ...
   and all of their `issuerRef` values in place. After the invariant-preserving
   canary succeeds, install the temporary production-`CertificateRequest`
   admission guard. While the Vault issuer remains active, finish every existing
   owned request and require each affected `Certificate` to be ready and not
   issuing with no pending request or staged next private-key Secret; abort if
   that state cannot be reached, leaving the Vault issuer unchanged and removing
   and verifying removal of the guard so Vault-backed issuance can recover. Then
   remove the issuer's delete-and-recreate hook behavior and convert the existing
   `vault-nico-issuer` object in place from `spec.vault` to
   `spec.ca.secretName: site-root`; the already-direct OTLP issuer remains
   unchanged. Remove the guard only after the issuer is ready and the object identity,
   Certificate revisions, Secret contents, and CertificateRequest counts remain
   unchanged. Otherwise, restore the Vault issuer spec with the guard held. Enable
...
independently of Vault retirement. A predecessor site's shipped shared-Secret
...
- The certificate enrollment authority must bind each approved public key and
  identity to the intended inventory record. Before a deployment elects to
  disable Scout and DPU-agent machine mTLS as a fallback authentication path,
  the API validation path enforces individual credential disablement or
  revocation through the CRL, OCSP, denylist, or equivalent mechanism selected by
  [#5956](https://github.com/dsx-ai-factory/infra-controller/issues/5956).
  This option does not affect the admin CLI's required mTLS authentication.
...
- The stable `vault-nico-issuer` compatibility identity must be converted in
  place to use the current CA certificate and signing key without changing any
  `Certificate.issuerRef`. A chain mismatch, unavailable signing key, replaced
  issuer object, undrained or unexpected `CertificateRequest`, staged next
  private-key Secret,
  insufficient remaining CA validity, or migration action that would force reissuance stops the cutover
...
- A compatible approver-policy controller is installed and ready before built-in
  ...
  stable compatibility issuer and the already-direct issuer during overlap and
  enforce every supported identity, SAN, usage, and lifetime profile before the
  non-Vault provider and issuer cutover become usable.
...
- Node certificate and key generations are validated and promoted atomically
  ...
  startup recovery from incomplete staging. All node certificate consumers use
  the shared generation-aware reader so one read cannot combine files from two
  generations during promotion.
- The retained CA signing Secret is not automatically renewed or rekeyed.
  ...
  overlap; root replacement occurs only through the coordinated rollover. Its
  protected backup is encrypted under separately held key material, access is
  audited, restore is tested before detachment, and retention continues through
  the signer's active lifetime and final post-retirement recovery window. Key
  loss without a usable backup enters emergency rollover rather than silent root
  replacement.
...
- install renewed node certificate/key pairs atomically and recover the prior
  valid generation after an interrupted promotion, without allowing a
  concurrent reader to combine two generations;
...
- restore the retained CA signing key from a tested protected backup or complete
  emergency key-loss rollover before existing leaves expire;
...
- roll forward from a Vault-backed deployment without credential loss, using
...
8. If the optional fTPM stretch goal is adopted, is its certificate installed
   ...
   The current `CertificateProvider` returns a server-generated private key, so
   API issuance for a key that cannot leave the fTPM would require a
   signing-request operation rather than the current interface. This remains a
   design option, not a selected approach.
...
This section is a status snapshot dated 2026-09-23. Sections 1 through 7 define
...
| Operator file/environment foundation | Implemented and documented, including file-backed UFM credentials with live reload and an authoritative local mode plus the BMC version-0 source selector and watched-Secret bootstrap | [#357](https://github.com/dsx-ai-factory/infra-controller/issues/357), [#1837](https://github.com/dsx-ai-factory/infra-controller/issues/1837), [#5732](https://github.com/dsx-ai-factory/infra-controller/issues/5732), [#6487](https://github.com/dsx-ai-factory/infra-controller/pull/6487), [Credential Sources](../configuration/credential-sources.md) |
...
| Admin CLI authentication | Open for the operator-managed non-Vault mTLS certificate lifecycle; DPF bootstrap no longer depends on the CLI | [#5955](https://github.com/dsx-ai-factory/infra-controller/issues/5955) |
...
| Non-Vault certificate provider | Open; cert-manager and the existing issuing CA are selected, with a new Rust Kubernetes-resource abstraction that uses in-memory keys and CSR-only `CertificateRequest` resources for dynamic node and UFM issuance, a stable `vault-nico-issuer` identity converted in place to direct signing, default-deny request approval, a static root signing Secret, validation-CA inputs moved to a separate overlap bundle, unchanged chart-managed certificate/key resources, no forced migration-time rotation, a fail-closed supported-predecessor and CA-validity check, post-cutover CA rollover and recovery, and API-side enforcement of individual node-signing credential disablement or revocation | [#5956](https://github.com/dsx-ai-factory/infra-controller/issues/5956) |
| Vault-optional installation and bootstrap | Open for the remaining chart defaults and runtime, REST, KMS, certificate, legacy Flow, and per-DPU ingestion-fence dependencies; the DPF BMC version-0 bootstrap path is already Vault-free | [#5958](https://github.com/dsx-ai-factory/infra-controller/issues/5958), [#6147](https://github.com/dsx-ai-factory/infra-controller/issues/6147) |
