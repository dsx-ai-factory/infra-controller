...
| 3 | site-explorer aborts every run: `MissingCredentials` | `machines/bmc/site/root` isn't in default kvSeeds; the seeded UEFI creds ship with **empty** passwords which fail validation | Script seeds the full chain, or enable `vault.siteCredentials` in `helm-prereqs/values.yaml` to seed the three site paths from the chart ([Seeding site credentials](https://github.com/dsx-ai-factory/infra-controller/blob/main/helm-prereqs/README.md#seeding-site-credentials)) |
