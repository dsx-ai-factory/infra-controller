...
Enabling `vault.siteCredentials` in `helm-prereqs/values.yaml` seeds all three
paths from the chart with non-empty passwords (see
[Seeding site credentials](https://github.com/dsx-ai-factory/infra-controller/blob/main/helm-prereqs/README.md#seeding-site-credentials));
otherwise `setup-machine-a-tron.sh` Phase 4 seeds them.
...
| site-explorer aborts with `MissingCredentials machines/bmc/site/root` | Site BMC root cred not in default `kvSeeds` | Seed `secrets/machines/bmc/site/root` = `root`/&lt;non-factory password&gt; in Vault, or enable `vault.siteCredentials` in `helm-prereqs/values.yaml` ([Seeding site credentials](https://github.com/dsx-ai-factory/infra-controller/blob/main/helm-prereqs/README.md#seeding-site-credentials)) |
