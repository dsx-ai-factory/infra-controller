...
func TestConfiguredPropagator(t *testing.T) {
	tcs := []struct {
		...
		{
			descr:  "default W3C trace-context and baggage propagators",
...
func TestBootstrapServiceName(t *testing.T) {
	tcs := []struct {
		...
		// Every name source omitted: the SDK's spec fallback must apply so the
		// exported resource never lacks service.name.
		{descr: "SDK default when every name source is omitted", wantServiceName: "unknown_service:"},
	}
	...
	for _, tc := range tcs {
		t.Run(tc.descr, func(t *testing.T) {
			...
			if tc.fallback == "" && tc.envServiceName == "" {
				assert.True(t, strings.HasPrefix(serviceName(res), tc.wantServiceName), "got %q", serviceName(res))
			} else {
