...
func TestFullIntegration(t *testing.T) {
	...
	assert.Equal(t, parentTraceID.String(), response.Header.Get(TraceIDHeader),
		"the response reports the trace the request joined")
...
func TestOriginalBehaviorMatch(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantHeader:      "aabbccdd000000000000000000000000",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.wantHeader, w.Result().Header.Get(TraceIDHeader),
				"the response header reports the trace id only when one exists")
		})
	}
