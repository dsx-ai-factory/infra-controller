...
func TracingInterceptor() (interceptor.Interceptor, error) {
	...
	otelInterceptor, err := opentelemetry.NewTracingInterceptor(opentelemetry.TracerOptions{
		...
		// Open workflows carry the tracing header written under the previous
		// propagator configuration. A header the current propagator cannot
		// read must start a new root, not fail the workflow task.
