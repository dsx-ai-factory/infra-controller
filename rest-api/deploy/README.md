...
| `tracing.serviceName` | `nico-rest-api` / `nico-rest-workflow` in both the binaries and the charts | `service.name` used when `OTEL_SERVICE_NAME` is not set. `OTEL_SERVICE_NAME` always wins. Every other binary falls back to the OpenTelemetry SDK default `unknown_service:<executable>` when no name is configured. |
...
W3C baggage travels the same paths, including Temporal, where the Temporal SDK
writes it into workflow headers. Those headers are persisted in workflow
history, so treat baggage as durable rather than request-scoped and keep
sensitive values out of it. Set `OTEL_PROPAGATORS=tracecontext` to propagate
trace context without baggage.
...
| `OTEL_PROPAGATORS` | `tracecontext,baggage` | Comma-separated list understood by the OpenTelemetry Go `autoprop` package (`tracecontext`, `baggage`, `b3`, `b3multi`, `jaeger`, `xray`, `ottrace`, `none`). Setting this replaces the default, so include `baggage` if you still want it. An unknown name fails the bootstrap. |
...
propagator makes the bootstrap return an error. The API and workflow binaries
refuse to start on that error; the other services log it and start without
tracing.
...
Every traced API response also carries the trace it belongs to in the
`X-Nico-Trace-Id` header, so you can look a specific request up directly:

```bash
curl -sS -D - -o /dev/null https://<api-host>/v1/sites | grep -i x-nico-trace-id
```
