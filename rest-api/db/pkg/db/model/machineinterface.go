...
func (micd MachineInterfaceSQLDAO) Create(ctx context.Context, tx *db.Tx, input MachineInterfaceCreateInput) (_ *MachineInterface, retErr error) {
	...
	ctx, machineInterfaceDAOSpan := cotel.StartSpan(ctx, "MachineInterfaceDAO.Create")
	defer func() { cotel.EndSpan(machineInterfaceDAOSpan, retErr) }()
...
func (micd MachineInterfaceSQLDAO) GetByID(ctx context.Context, tx *db.Tx, id uuid.UUID,
	...
	// Create a child span and set the attributes for current request
	ctx, machineInterfaceDAOSpan := cotel.StartSpan(ctx, "MachineInterfaceDAO.GetByID")
	defer func() { cotel.EndSpan(machineInterfaceDAOSpan, retErr) }()
	cotel.SetAttribute(machineInterfaceDAOSpan, attribute.String("id", id.String()))
...
func (micd MachineInterfaceSQLDAO) GetAll(ctx context.Context, tx *db.Tx, filter MachineInterfaceFilterInput, page paginator.PageInput, includeRelations []string) (_ []MachineInterface, _ int, retErr error) {
	// Create a child span and set the attributes for current request
	ctx, machineInterfaceDAOSpan := cotel.StartSpan(ctx, "MachineInterfaceDAO.GetAll")
	defer func() { cotel.EndSpan(machineInterfaceDAOSpan, retErr) }()
...
func (micd MachineInterfaceSQLDAO) Update(
	...
	// Create a child span and set the attributes for current request
	ctx, machineInterfaceDAOSpan := cotel.StartSpan(ctx, "MachineInterfaceDAO.UpdateFromParams")
	defer func() { cotel.EndSpan(machineInterfaceDAOSpan, retErr) }()
	...
	if input.ControllerInterfaceID != nil {
		...
		cotel.SetAttribute(machineInterfaceDAOSpan, attribute.String("controller_interface_id", input.ControllerInterfaceID.String()))
	}
	if input.ControllerSegmentID != nil {
		...
		cotel.SetAttribute(machineInterfaceDAOSpan, attribute.String("controller_segment_id", input.ControllerSegmentID.String()))
	}
	if input.AttachedDpuMachineID != nil {
		...
		cotel.SetAttribute(machineInterfaceDAOSpan, attribute.String("attached_dpu_machine_id", *input.AttachedDpuMachineID))
	}
	if input.SubnetID != nil {
		m.SubnetID = input.SubnetID
		updatedFields = append(updatedFields, "subnet_id")
		cotel.SetAttribute(machineInterfaceDAOSpan, attribute.String("subnet_id", input.SubnetID.String()))
	}
	if input.Hostname != nil {
		m.Hostname = input.Hostname
		updatedFields = append(updatedFields, "hostname")
		cotel.SetAttribute(machineInterfaceDAOSpan, attribute.String("hostname", *input.Hostname))
	}
	...
	if input.MacAddress != nil {
		...
		cotel.SetAttribute(machineInterfaceDAOSpan, attribute.String("mac_address", *input.MacAddress))
	}
...
func (micd MachineInterfaceSQLDAO) Clear(ctx context.Context, tx *db.Tx, input MachineInterfaceClearInput) (_ *MachineInterface, retErr error) {
	// Create a child span and set the attributes for current request
	ctx, machineInterfaceDAOSpan := cotel.StartSpan(ctx, "MachineInterfaceDAO.ClearFromParams")
	defer func() { cotel.EndSpan(machineInterfaceDAOSpan, retErr) }()
...
func (micd MachineInterfaceSQLDAO) Delete(ctx context.Context, tx *db.Tx, id uuid.UUID, purge bool) (retErr error) {
	// Create a child span and set the attributes for current request
	ctx, machineInterfaceDAOSpan := cotel.StartSpan(ctx, "MachineInterfaceDAO.Delete")
	defer func() { cotel.EndSpan(machineInterfaceDAOSpan, retErr) }()
	cotel.SetAttribute(machineInterfaceDAOSpan, attribute.String("id", id.String()))
