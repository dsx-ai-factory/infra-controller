...
func init() {
	Migrations.MustRegister(func(ctx context.Context, db *bun.DB) error {
		...
		handleError(tx, err)
		_, err = tx.Exec("ALTER TABLE expected_rack_group DROP CONSTRAINT IF EXISTS expected_rack_group_group_id_site_id_key")
		handleError(tx, err)
		...
		handleError(tx, err)
		_, err = tx.Exec("DROP INDEX IF EXISTS expected_rack_group_site_id_idx")
		handleError(tx, err)
		_, err = tx.Exec("CREATE INDEX expected_rack_group_site_id_idx ON expected_rack_group(site_id)")
		handleError(tx, err)
		_, err = tx.Exec("DROP INDEX IF EXISTS expected_rack_group_created_idx")
		handleError(tx, err)
