...
type InstanceUpdateRequest struct {
	...
	// Update SpectrumX Partition attachments of the Instance. Omission or null leaves attachments unchanged; an explicit list replaces them entirely. An empty list removes attachments without capability validation. A nonempty list, including an unchanged list, is validated against the Instance's Machine capabilities persisted by inventory reconciliation before any update is persisted. Missing capabilities or invalid selectors return 400. No inline Site inventory lookup is performed. Final Site validation remains authoritative because persisted inventory can lag behind the Site. Each `device` and `deviceInstance` pair may appear only once, irrespective of `virtualFunctionId`.
