...
func TestGetLabelKeyFromPath(t *testing.T) {
	tests := []struct {
		name    string
		param   string
		path    string
		...
		{name: "literal percent", param: "100%", want: "100%"},
		{name: "router decodes percent-encoded slash", path: "/label/key/nvidia.com%2Ffailure-domain/value", want: "nvidia.com/failure-domain"},
		{name: "router decodes percent-encoded percent", path: "/label/key/100%25/value", want: "100%"},
		{name: "router decodes only once", path: "/label/key/a%2541b/value", want: "a%41b"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var gotErr error
			if tt.path != "" {
				e.GET("/label/key/:key/value", func(c echo.Context) error {
					got, gotErr = getLabelKeyFromPath(c)
				...
				got, gotErr = getLabelKeyFromPath(ec)
			}
