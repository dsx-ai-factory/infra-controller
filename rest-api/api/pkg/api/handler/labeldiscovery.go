...
// getLabelKeyFromPath decodes and validates the label key path parameter.
func getLabelKeyFromPath(c echo.Context) (string, error) {
	labelKey := c.Param("key")
	if c.Request().URL.RawPath != "" {
		var err error
		labelKey, err = url.PathUnescape(labelKey)
		if err != nil {
			return "", err
		}
	}
...
func handleExpectedMachineLabelList(c echo.Context, dbSession *cdb.Session, tracerSpan *cutil.TracerSpan, kind labelListKind) error {
	...
	// Match GetAllExpectedMachineHandler: dual-role callers receive the union
	// of Provider-owned Sites and Sites available through their Tenant role.
...
func handleMachineLabelList(c echo.Context, dbSession *cdb.Session, tracerSpan *cutil.TracerSpan, kind labelListKind) error {
	...
	// Match GetAllMachineHandler: the Provider scope takes precedence for a
	// dual-role caller, so Tenant Sites owned by other Providers are excluded.
...
func bindLabelListRequest(c echo.Context, kind labelListKind) (model.APILabelGetAllRequest, pagination.PageRequest, string, *cutil.APIError) {
	...
	if kind == labelListValues {
		labelKey, err = getLabelKeyFromPath(c)
		if err != nil {
