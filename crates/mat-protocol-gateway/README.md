...
`tests/source_list_contract.rs` includes the same file and checks that the
...
3. Configure the UFM reconciliation with one static source per entry
   (`base_url` + `sources.status_path`) and mark the gateway ready.
...
| `sources.status_path` | string | `/machines/status` | Appended to each `base_url`; must start with `/` |
