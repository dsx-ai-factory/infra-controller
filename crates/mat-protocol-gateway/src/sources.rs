    ...
    pub fn inventory_sources(&self, client: &SourceClientConfig) -> Vec<InventorySourceConfig> {
        self.sources
            ...
            .map(|source| InventorySourceConfig {
                url: source.status_url(&client.status_path),
...
impl Source {
    /// Joins the status path onto the base URL, keeping any path prefix the base URL carries.
    fn status_url(&self, status_path: &str) -> Url {
...
mod tests {
    ...
    /// `tests/source_list_contract.rs`; these tests use it as realistic input.
    ...
    fn status_url_preserves_base_path_prefix() {
        ...
        assert_eq!(
            source.status_url("/machines/status").as_str(),
