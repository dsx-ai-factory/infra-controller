...
/// Path appended to each discovered machine-a-tron base URL to obtain its inventory snapshot.
const DEFAULT_STATUS_PATH: &str = "/machines/status";
...
impl GatewayConfig {
    ...
    /// Rejects settings that cannot become active: zero intervals, no startup attempts, a
    /// relative status path, or UFM options the gateway owns itself.
    pub fn validate(&self) -> eyre::Result<()> {
        ...
        eyre::ensure!(
            self.sources.status_path.starts_with('/'),
            "sources.status_path must start with '/'"
        );
        Ok(())
    }
}
...
pub struct SourceClientConfig {
    /// Absolute path appended to each `base_url`. Defaults to `/machines/status`.
...
mod tests {
    ...
    fn empty_configuration_uses_documented_defaults() {
        ...
        assert_eq!(config.sources.status_path, DEFAULT_STATUS_PATH);
