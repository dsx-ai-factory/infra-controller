...
async fn both_services_are_mounted() {
    ...
    // reports as `Unimplemented` -- indistinguishable from a handler that
    // deliberately returns it. So each service is checked by a call that only
    // its own handler could answer.
...
/// The fabric configuration NICo sends: the rack profile's topology and no
...
/// The response shape NICo requires before it will keep going. Both of the
/// ways this can silently strand a switch are asserted: a per-node job id that
/// is missing or empty makes NICo give up with "succeeded but returned no job
/// id", and a batch whose status, node results and stats disagree is read as
/// failure.
#[tokio::test]
...
/// Jobs must actually reach a terminal state, and must spell it the way the
/// caller's vocabulary expects. An unrecognised spelling is read as
/// "in progress", so a typo here is an infinite poll rather than an error.
#[tokio::test]
...
/// job-status RPCs and as a normal response rather than a gRPC error. NICo
/// persists job ids in its database, so after the mock's host restarts it
/// polls for such jobs, and failing them would strand the switch.
#[tokio::test]
...
/// A request naming no switches is rejected synchronously, as the proto
...
/// `ConfigureScaleUpFabricManager` has no per-node results, so a request whose
/// switches all miss the inventory cannot fail per node the way the batch RPCs
/// do. Nor may it complete: the caller would read the fabric back and wait
/// forever for a primary that can never be elected. The proto reserves
/// `INVALID_ARGUMENT` for a malformed config, so the request is accepted and
/// its job fails naming the switches, which the caller records as the rack's
/// error. One matched switch is enough for the job to complete.
#[tokio::test]
async fn a_fabric_request_matching_no_switch_returns_a_job_that_fails() {
    ...
    for case in cases {
        ...
        // A failing job is paced like a completing one and stays where it
        // ended; its reason appears only once it has failed.
...
/// After the fabric manager is configured, the caller reads the fabric back and
/// requires exactly one enabled switch per rack; several enabled switches are
/// reported as "multiple primary switches" and the rack never reaches ready.
#[tokio::test]
async fn fabric_status_reports_exactly_one_primary_per_configured_rack() {
    ...
    // No primary requested: exactly one switch is elected, the one lowest in
    // the rack rather than the lowest node id.
    ...
    // A requested primary that belongs to the rack is honoured over the one
    // the default election picks, and replaces it rather than joining it.
    v2.configure_scale_up_fabric_manager(
        librms::protos::rack_manager_v2::ConfigureScaleUpFabricManagerRequest {
            nodes: Some(node_set()),
...
/// A switch the request names but the mock does not have cannot run the
/// fabric manager. It is never elected, whether it sorts first, is the
/// requested primary, or the rack is first read without having been
/// configured; the election falls to a switch the caller can reach, and the
/// unreachable one is listed with an error.
#[tokio::test]
...
/// A node no simulated device answers for is a per-node failure on every
/// batch RPC, the way `BatchGetNodeDeviceInfo` already reports one: the batch
/// fails, the node's result says why, the stats count it, and no job is
/// issued for it. NICo records the per-node error instead of polling a job
...
/// A caller may read the fabric without this process having seen the rack's
/// configuration call (the mock's host restarted, or an unknown job was
/// reported complete). It still expects exactly one primary, and only the
/// primary reports a configured fabric-manager control plane.
#[tokio::test]
...
async fn fabric_membership_reads_back_what_was_written() {
    ...
    // The lone switch would otherwise be elected and read back enabled.
    client
        .batch_set_scale_up_fabric_state(
            librms::protos::rack_manager::BatchSetScaleUpFabricStateRequest {
                nodes: Some(node_set()),
...
/// Two racks configured at the same time, from separate tasks on a
/// multi-threaded runtime, each end up with exactly one primary of their own:
/// election state is kept per rack and one rack's choice never leaks into
/// another's.
...
async fn fabric_election_is_independent_per_rack() {
    ...
    // One request per rack from its own task, as a controller working several
    // racks at once issues them.
    let configure =
