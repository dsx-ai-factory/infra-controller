...
fn validate_extension_service_identities(
    ...
    let attachment_ids: HashSet<_> = config
        .service_configs
        .iter()
        .map(|service| service.id)
        .collect();
    if attachment_ids.contains(&uuid::Uuid::nil())
        || attachment_ids.len() != config.service_configs.len()
    {
        return Err(invalid_extension_service_identity(
            "extension-service attachment IDs must be non-nil and unique within an instance",
        ));
    }
...
mod tests {
    ...
    /// Verifies the snapshot boundary requires migrated IDs instead of inventing
    /// an identity while reading an attachment written by an older binary.
    #[test]
    fn extension_service_attachment_requires_persisted_id() {
        // Recreate the legacy JSON at the actual snapshot deserialization boundary.
        let mut persisted = serde_json::to_value(minimal_pg_json()).expect("serialize snapshot");
        ...
        assert!(serde_json::from_value::<InstanceSnapshotPgJson>(persisted).is_err());
    }
    ...
    fn networked_extension_service_identity_is_validated() {
        ...
        scenarios!(
            run = |(ids, endpoint_owner): (Vec<Uuid>, Uuid)| {
                ...
                let network = InstanceNetworkConfig {
                    service_interfaces: vec![service_endpoint(endpoint_owner)],
                    ..Default::default()
                };
                validate_extension_service_identities(&config, &network).map_err(drop)
            };
            "stored ownership" {
                // A persisted owner must remain usable without any identity normalization.
                (vec![attachment_id], attachment_id) => Yields(()),
                // Nil is a syntactically valid UUID but cannot identify an attachment.
                (vec![Uuid::nil()], Uuid::nil()) => Fails,
                // Duplicate owners make endpoint correlation ambiguous.
                (vec![attachment_id, attachment_id], attachment_id) => Fails,
                // A valid endpoint UUID still needs an attachment in this instance.
                (vec![attachment_id], Uuid::new_v4()) => Fails,
            }
        );
    }
