...
pub(crate) async fn create(
    ...
    require_recorded_class(&mut txn, &new_profile.hardware_class).await?;
...
/// Refuses a new profile keyed to a class no endpoint reports, so a misspelling
/// fails at the point of writing rather than sitting there looking applied.
///
/// Only create is gated. Update and delete gate on the row existing instead, so
/// a profile whose hardware has left the site stays editable and removable.
/// `any` is exempt: it is never recorded on an endpoint, and seeding it is the
/// first thing an operator does.
async fn require_recorded_class(
    txn: &mut sqlx::PgConnection,
    hardware_class: &str,
) -> Result<(), CarbideError> {
    if hardware_class == ANY_HARDWARE_CLASS
        || db::explored_endpoints::hardware_class_recorded(txn, hardware_class).await?
    {
        return Ok(());
    }

    Err(CarbideError::InvalidArgument(format!(
        "no explored endpoint reports hardware class '{hardware_class}'; \
         run 'attestation spdm coverage' for the classes this site has"
    )))
}
