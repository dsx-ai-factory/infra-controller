...
use model::attestation::profile::{ANY_HARDWARE_CLASS, validate_hardware_class};
...
impl Create {
    async fn execute(
        ...
        // The server refuses this too, and additionally that no endpoint
        // reports the class, which only it can see. Checking the shape here
        // costs a malformed class a local error rather than a round trip.
        validate_hardware_class(&self.hardware_class)
