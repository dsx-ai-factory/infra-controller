...
async fn only_edits_that_stop_attesting_unprofiled_hardware_require_force() {
    for (args, reaches_server) in [
        ...
        (&["update", HARDWARE_CLASS, "--mode", "none"][..], true),
        (&["delete", "any"][..], false),
        (&["delete", "any", "--force"][..], true),
        (&["delete", HARDWARE_CLASS][..], true),
    ] {
...
/// A class is only ever the shape exploration derives, so one that could not
/// have been derived costs a local error rather than a round trip. Whether an
/// endpoint reports it is the server's to judge, and this checks nothing about
/// it.
#[tokio::test]
async fn create_refuses_a_hardware_class_the_derivation_could_not_produce() {
    let client = FakeClient::default();
    let (result, _) = execute(
        &["create", "DGX GB200", "--mode", "all"],
        &client,
    ...
    let error = result
        .expect_err("a malformed class is not a profile key")
        .to_string();
    assert!(error.contains("DGX GB200"), "{error}");
    assert!(client.requests.borrow().is_empty(), "{error}");

    for hardware_class in [HARDWARE_CLASS, ANY_HARDWARE_CLASS] {
        let client = FakeClient::default();
        let (result, _) = execute(
...
async fn the_profile_table_shows_each_profile_with_the_version_to_match() {
    ...
    let (result, output) = execute(&["get", HARDWARE_CLASS], &client, OutputFormat::Json).await;
    result.unwrap();
