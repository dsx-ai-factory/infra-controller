...
pub(super) async fn handle_show_reserved_addresses(
    ...
    let rows: Vec<ReservedAddressRow> = resp
        .reserved_addresses
        .iter()
        .map(|reserved| ReservedAddressRow {
            ...
            allocation_type: reserved.allocation_type.clone(),
        })
        .collect();
    ...
    match output_format {
        ...
        _ => {
            if rows.is_empty() {
                println!("No reserved addresses found");
            } else {
                let mut table = Table::new();
                table.set_titles(Row::new(vec![
                    Cell::new("Address"),
                    Cell::new("Reserved By MAC"),
                    Cell::new("Family"),
                    Cell::new("Type"),
                ]));
