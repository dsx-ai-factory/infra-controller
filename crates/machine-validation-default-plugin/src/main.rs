...
fn run(input_path: &str, output_path: &str) -> Result<(), String> {
    let input: Input = serde_json::from_slice(&fs::read(input_path).map_err(|e| format!("read input: {e}"))?)
    ...
    let result = if findings.is_empty() {
        ResultFile { contract_version: "v1", kind: "MachineValidationPluginResult", outcome: "pass", severity: "info", summary: "all basic checks passed".to_owned(), findings }
    } else {
        ResultFile { contract_version: "v1", kind: "MachineValidationPluginResult", outcome: "fail", severity: "critical", summary: "one or more basic checks failed".to_owned(), findings }
    };
...
fn write_result(path: &str, result: &ResultFile<'_>) -> Result<(), String> {
    ...
    fs::write(&temporary, serde_json::to_vec(result).map_err(|e| format!("serialize result: {e}"))?)
...
mod tests {
    ...
    fn accepts_dcgm_check_levels() {
        for level in [1, 3] {
            let value: Input = serde_json::from_str(&format!(r#"{{"contractVersion":"v1","kind":"MachineValidationPluginInput","parameters":{{"checks":[{{"name":"dcgm-diagnostic","parameters":{{"runLevel":{level}}}}]}}}}"#)).unwrap();
    ...
    fn rejects_unknown_parameters() {
        assert!(serde_json::from_str::<RequestedCheck>(r#"{"name":"dcgm-diagnostic","unexpected":true}"#).is_err());
    }
}
