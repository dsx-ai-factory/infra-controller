...
fn sensitive_values(credentials: &BmcCredentials) -> Arc<[String]> {
    ...
    match credentials {
        BmcCredentials::UsernamePassword { username, password } => {
            // Retain the plaintext password when present and always retain the
            // exact Basic header generated by nv-redfish's reqwest client.
...
mod tests {
    ...
    /// Verifies nv-redfish removes both plaintext and derived Basic credentials
    /// from its returned error and its single structured diagnostic.
    #[test]
    fn http_failure_logs_structured_context_and_masks_credentials() {
        // Build the client with Basic credentials and construct the exact wire
        // value that a hostile or broken BMC may echo in its error document.
            ...
            text: format!(
                ...
                "error": {{
                    "@Message.ExtendedInfo": [{{
                        "Message": "credential s\u0065cret or {basic_authorization} rejected"
                    }}]
                }}
            }}"#,
            ),
