...
/// a number, `true`, `false`, `null`, or a bare RFC 3339 instant.
...
impl Filter {
    pub(crate) fn parse(expression: &str) -> Result<Self, String> {
        ...
        match parser.tokens.get(parser.at) {
            None => Ok(filter),
            Some(Token::Close) => Err(parser.error("unbalanced `)`")),
    ...
    pub(crate) fn admits(&self, resource: &Value) -> bool {
        match self {
            Filter::Compare {
                path,
                operator,
...
/// comparable, which only `ne` counts as satisfied. Two strings that are both
/// RFC 3339 instants order as instants, so a `Created` in one offset compares
/// correctly against a literal in another.
fn compare(actual: &Value, literal: &Literal) -> Option<Ordering> {
    match (actual, literal) {
        (Value::String(actual), Literal::Str(literal)) => match (instant(actual), instant(literal))
        {
            (Some(actual), Some(literal)) => Some(actual.cmp(&literal)),
            _ => Some(actual.as_str().cmp(literal.as_str())),
        },
...
impl std::fmt::Display for Token {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Token::Quoted(text) => write!(f, "'{text}'"),
            ...
            '\'' => {
                ...
                loop {
                    match chars.next() {
                        ...
                        None => {
                            return Err(format!(
                                "unsupported $filter `{expression}`: unterminated string literal"
                            ));
                        }
                    }
                }
...
impl Parser<'_> {
    fn error(&self, detail: &str) -> String {
        format!("unsupported $filter `{}`: {detail}", self.expression)
    }
    ...
    fn unary(&mut self) -> Result<Filter, String> {
        match self.next() {
            ...
            Some(token) => Err(self.error(&format!("expected a property name, found `{token}`"))),
            None => Err(self.error("expected a property name")),
        }
    }

    fn comparison(&mut self, property: String) -> Result<Filter, String> {
        let path: Vec<String> = property.split('/').map(str::to_owned).collect();
        if path.iter().any(String::is_empty) {
            return Err(self.error(&format!("`{property}` is not a property path")));
        }
        ...
        .ok_or_else(|| {
            self.error(&format!(
                "expected one of eq, ne, gt, ge, lt, le after `{property}`"
            ))
        })?;
    ...
    fn literal(&mut self, property: &str) -> Result<Literal, String> {
        match self.next() {
            Some(Token::Quoted(text)) => Ok(Literal::Str(text)),
            Some(Token::Word(word)) => {
                // A `+` in an instant's offset arrives as a space once the
                // query is decoded, splitting the instant in two.
                ...
                word_literal(&word).ok_or_else(|| {
                    self.error(&format!(
                        "`{word}` is not a quoted string, number, boolean, null, or RFC 3339 instant"
                    ))
                })
            }
            _ => Err(self.error(&format!("expected a literal to compare `{property}` with"))),
        }
    }
}
...
fn word_literal(word: &str) -> Option<Literal> {
    Some(match word {
        ...
        "null" => Literal::Null,
        _ => {
            if let Some(number) = word.parse::<f64>().ok().filter(|number| number.is_finite()) {
...
mod tests {
    ...
    fn admits(expression: &str, resource: &Value) -> Result<bool, String> {
    ...
    fn malformed_expressions_are_rejected() {
        for expression in [
            ...
            "Created ge yesterday",
            "Created ge 'unterminated",
            "(Created ge 2026-02-12T02:06:58Z",
            "Created ge 2026-02-12T02:06:58Z)",
            "Created ge 2026-02-12T02:06:58Z Severity eq 'OK'",
            "Created/ ge 1",
        ] {
            let error = Filter::parse(expression).unwrap_err();
            assert!(
                error.starts_with(&format!("unsupported $filter `{expression}`: ")),
                "{error}"
            );
        }
    }
