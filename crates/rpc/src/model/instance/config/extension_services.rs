...
impl TryFrom<rpc::InstanceDpuExtensionServicesConfig> for InstanceExtensionServicesConfig {
    ...
    fn try_from(config: rpc::InstanceDpuExtensionServicesConfig) -> Result<Self, Self::Error> {
        Ok(RequestedInstanceExtensionServicesConfig::try_from(config)?.into_new_attachments())
    }
}

/// Builds the caller-visible active attachment list and restores each VPC selection
/// from stored endpoints.
pub(super) fn to_rpc_config(
    ...
    for attachment in config.service_configs {
        ...
        for endpoint in service_interfaces
            .iter()
            .filter(|endpoint| endpoint.attachment_id == attachment.id)
        {
                ...
                && existing != endpoint.vpc_id
            {
                return Err(RpcDataConversionError::InvalidValue(
                    ...
                    format!(
                        "attachment {} has inconsistent VPCs for interface ordinal {}",
                        attachment.id, endpoint.interface_ordinal
                    ),
                ));
            }
        }
                ...
                .is_some_and(|(&ordinal, _)| ordinal != 0)
        {
            return Err(RpcDataConversionError::InvalidValue(
                "service_vpc_ids".to_string(),
                format!(
                    "attachment {} has unsupported interface ordinals",
                    attachment.id
                ),
            ));
        }
...
mod tests {
    ...
    /// Verifies public readback reports one selected VPC only when every DPU
    /// endpoint for that service interface agrees on the stored selection.
    #[test]
    fn service_vpc_readback_requires_dpu_consensus() {
        // Build matching endpoint copies for two DPUs in one attachment.
