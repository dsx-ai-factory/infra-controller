...
if [[ "$_work_dir_existed" == true ]]; then
    ...
    # stderr is redirected to the log at this point, so `read -p` would hide the
    # question. Ask on the terminal when there is one; otherwise read stdin as
    # before this prompt moved to /dev/tty, so `echo y | ssh host bash install.sh`
    # still works and an EOF still ends in "Aborted." instead of a bash error.
