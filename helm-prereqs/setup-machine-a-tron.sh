...
#    default nico-prereqs kvSeeds and the chart seeds it only when
#    vault.siteCredentials.enabled is set (off by default), so without it
#    site-explorer aborts every run with MissingCredentials and never explores
#    anything. Phase 4 seeds it when it is absent or still at a factory value.
#
