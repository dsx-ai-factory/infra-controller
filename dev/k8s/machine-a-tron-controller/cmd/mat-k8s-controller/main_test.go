...
func TestParseOptions_DefaultsAndEnvironment(t *testing.T) {
	...
	opts, err = parseOptions([]string{"--sync-interval=10s"}, func(k string) string { return env[k] })
	require.NoError(t, err)
...
func TestParseOptions_RejectsInvalidValues(t *testing.T) {
	for _, tc := range []struct {
		...
		{"zero sync interval", []string{"--sync-interval=0"}, "sync-interval must be positive", false},
		{"negative sync interval", []string{"--sync-interval=-1s"}, "sync-interval must be positive", false},
		{"negative debounce", []string{"--source-list-debounce=-1s"}, "source-list-debounce must not be negative", false},
		{"negative health bound", []string{"--health-stale-after=-1s"}, "health-stale-after must not be negative", false},
		{"unknown flag", []string{"--source-list-address=127.0.0.1:1"}, "flag provided but not defined", true},
	} {
		t.Run(tc.name, func(t *testing.T) {
			_, err := parseOptions(tc.args, noEnv)
			require.Error(t, err)
	...
	// A zero debounce and a zero health bound are valid (both disable a check).
	opts, err := parseOptions([]string{"--source-list-debounce=0", "--health-stale-after=0"}, noEnv)
	require.NoError(t, err)
